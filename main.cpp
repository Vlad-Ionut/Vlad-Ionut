...
public:
  volatile bool handleButtonPressNextTick = false; // Flag pentru gestionarea apăsării butonului în următorul ciclu

  Button(int p, uint8_t t, unsigned long debounce) : pin(p), triggerType(t), debounceDelayMs(debounce) {
    pinMode(pin, INPUT_PULLUP); // Setăm pinul ca intrare cu rezistență pull-up activă
  }

  bool isOnCooldown() const { return millis() - lastInterruptTime < debounceDelayMs; } // Verificăm dacă butonul este în cooldown

  void resetCooldown() {
    handleButtonPressNextTick = true; // Pregătim butonul pentru gestionarea presării în următorul ciclu
    lastInterruptTime = millis(); 
  }

  virtual void onPress() { handleButtonPressNextTick = false; } // Resetăm flag-ul de presare
};

// Clasele pentru butonul de Start/Stop și pentru Dificultate
...
// Instante
RGBLED rgbLed(4, 5, 6); //// Pinii pentru LED-ul RGB
...
// ISR pentru Timer1 pentru generarea cuvintelor
ISR(TIMER1_COMPA_vect) {
  ...
  if (gameStarted) {
    ...
    if (counter >= timerOverflowCounter) {
      generateRandomWord();
      counter = 0; //resetam contorul dupa generare cuvantului
    }
  }
}
...
void StartStopButton::onPress() {
  Button::onPress();
  if (!gameStarted && !countdownActive) {// Dacă jocul nu a început și nu este în numărătoare inversă
    startCountdown(); 
  } else if (gameStarted) {//dc e in desf il oprim
    gameStarted = false;
    rgbLed.turnWhite();
